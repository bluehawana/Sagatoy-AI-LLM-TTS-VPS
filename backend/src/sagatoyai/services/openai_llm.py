"""OpenAI LLM service for fast inference using GPT-4o mini."""
...
class OpenAILLMService:
    """OpenAI service for LLM inference using GPT-4o mini."""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data, headers=headers) as response:
                    if response.status != 200:
                        ...
                        raise RuntimeError(f"OpenAI LLM failed: {response.status} - {error_text}")
