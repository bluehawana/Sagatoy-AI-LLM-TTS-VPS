"""NVIDIA Nemotron Voice Chat Test - Following test_groq.py pattern."""
...
class TestNemotronVoiceChat:
    """Test NVIDIA Nemotron for toy voice chat responses."""
    ...
    async def test_basic_math(self):
        ...
        for lang, question in test_cases:
            start = time.time()
            response, intent = await nemotron_service.generate_conversation_response(
    ...
    async def test_ask_for_song(self):
        ...
        for lang, question in test_cases:
            response, _ = await nemotron_service.generate_conversation_response(
                question, language=lang
            )
...
class TestNemotronEndpoint:
    """Test NVIDIA API endpoint availability."""

    @pytest.mark.asyncio
    async def test_api_available(self):
        """Test if Nemotron API is accessible."""
        from sagatoyai.services.nemotron import NemotronService
        
        service = NemotronService()
        assert service.api_key is not None or os.getenv("NVIDIA_API_KEY"), "No API key"

    @pytest.mark.asyncio
    async def test_fallback_chain(self):
        """Test that fallback to other LLMs works when Nemotron fails."""
        # This tests the fallback logic in mqtt_service
        # When Nemotron fails, should fall back to OpenAI -> Groq -> Z.ai
        pass
